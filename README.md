...
Workflow запускается ежедневно в 06:40 по времени Asia/Qyzylorda и повторяет попытку ровно через 12 часов, в 18:40. Каждый запуск входит в аккаунт и пытается получить награду. Если награда уже получена или пока недоступна, запуск завершается без ошибки, а следующая попытка выполняется через 12 часов. GitHub может начать запланированное задание на несколько минут позже.
